...
// Entity head: ASCII capital word OR CJK sequence (Chinese/Japanese/Korean)
const E = "(?:[A-Z]\\w+|[\\u4e00-\\u9fff\\u3040-\\u309f\\u30a0-\\u30ff\\uac00-\\ud7af]{1,15})";

const EXTRACTION_PATTERNS = [
  // ─── English patterns ───
  // "X's doctor is Y"
  { re: new RegExp(`^(${E})(?:'s)\\s+doctor\\s+is\\s+(.+)$`, "i"), r: "has_doctor" },
  // "X's lawyer is Y"
  { re: new RegExp(`^(${E})(?:'s)\\s+(?:lawyer|attorney)\\s+is\\s+(.+)$`, "i"), r: "has_lawyer" },
  // "X lives in Y"
  { re: new RegExp(`^(${E})\\s+lives?\\s+in\\s+([A-Z][\\w\\s,]+)$`, "i"), r: "lives_in" },
  // "X works at Y"
  { re: new RegExp(`^(${E})\\s+works?\\s+at\\s+([A-Z][\\w\\s&.]+)$`, "i"), r: "works_at" },
  // "X owns Y"
  { re: new RegExp(`^(${E})\\s+owns?\\s+(?:a\\s+)?([A-Z][\\w\\s]+)$`, "i"), r: "owns" },
  // "X drives a Y"
  { re: new RegExp(`^(${E})\\s+drives?\\s+(?:a\\s+)?([A-Z][\\w\\s]+)$`, "i"), r: "owns" },
  // "X has chronic Y" / "X has Y disease/condition"
  { re: new RegExp(`^(${E})\\s+has\\s+(?:chronic\\s+)?(\\w[\\w\\s]*(?:disease|condition|syndrome|urticaria|diabetes|asthma))$`, "i"), r: "has_condition" },
  // "X takes Y" (medication)
  { re: new RegExp(`^(${E})\\s+takes?\\s+([A-Z][\\w\\s]+\\d+\\s*mg)$`, "i"), r: "treated_by" },
  // "X attends Y"
  { re: new RegExp(`^(${E})\\s+attends?\\s+([A-Z][\\w\\s]+)$`, "i"), r: "attends" },
  // "X's wife/husband is Y"
  { re: new RegExp(`^(${E})(?:'s)\\s+(?:wife|husband)\\s+is\\s+(.+)$`, "i"), r: "spouse" },
  // "X's son/daughter is Y"
  { re: new RegExp(`^(${E})(?:'s)\\s+(?:son|daughter)\\s+is\\s+(.+)$`, "i"), r: "has_child" },

  // ─── Chinese patterns ───
  // "X的医生是Y" / "X的大夫是Y"
  { re: new RegExp(`^(${E})的(?:医生|大夫)是(.+)$`), r: "has_doctor" },
  // "X的律师是Y"
  { re: new RegExp(`^(${E})的律师是(.+)$`), r: "has_lawyer" },
  // "X住在Y"
  { re: new RegExp(`^(${E})住在(.+)$`), r: "lives_in" },
  // "X在Y工作" / "X在Y上班"
  { re: new RegExp(`^(${E})在(.+?)(?:工作|上班)$`), r: "works_at" },
  // "X有一辆Y" / "X有一台Y" / "X开Y"
  { re: new RegExp(`^(${E})(?:有一辆|有一台|开的是|开)(.+)$`), r: "owns" },
  // "X得了Y" / "X有慢性Y" / "X患有Y"
  { re: new RegExp(`^(${E})(?:得了|有慢性|患有|患|得)(.+?(?:病|症|炎|症候群|荨麻疹|糖尿病|哮喘|高血压))$`), r: "has_condition" },
  // "X吃Y" / "X服用Y" — medication with mg
  { re: new RegExp(`^(${E})(?:吃|服用|每天吃|每天服用)(.+\\d+\\s*mg)$`, "i"), r: "treated_by" },
  // "X上Y" (school) / "X在Y上学"
  { re: new RegExp(`^(${E})在(.+?)(?:上学|读书)$`), r: "attends" },
  // "X的妻子是Y" / "X的老婆是Y" / "X的丈夫是Y" / "X的老公是Y"
  { re: new RegExp(`^(${E})的(?:妻子|老婆|丈夫|老公|爱人)是(.+)$`), r: "spouse" },
  // "X的儿子是Y" / "X的女儿是Y" / "X的孩子是Y"
  { re: new RegExp(`^(${E})的(?:儿子|女儿|孩子)是(.+)$`), r: "has_child" },
];
